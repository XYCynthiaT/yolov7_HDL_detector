...
def run(
        ...
        pixel_per_nm=1, # tthe scale between pixel and nm
):
    source = str(source)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                d = torch.sqrt(pixel_count/torch.tensor([math.pi], device = pixel_count.device))*2
                incomplete = (torch.cat((x0, y0), 1) < d[:, None]).any(1) # dim = 1
                det = det[torch.logical_not(incomplete)]
                # print(f'det.shape = {det.shape}')
                masks = masks[torch.logical_not(incomplete)]
                d = d[torch.logical_not(incomplete)]
                ...
                for *xyxy, conf, cls in reversed(det[:, :6]):
                    if save_txt:  # Write to file
                        ...
                        dplx = d.tolist()[::-1][instance_id]
                        # if save_conf and toNM:
                        #    line = (cls, *xywh, conf, instance_id, dplx, dplx/pixel_per_nm) 
                        # elif not save_conf and toNM:
                        #    line = (cls, *xywh, instance_id, dplx, dplx/pixel_per_nm) 
                        # elif save_conf and not toNM:
                        #     line = (cls, *xywh, conf, instance_id, dplx) 
                        # else:
                        #    line = (cls, *xywh, instance_id, dplx) 
                        line = (cls, *xywh, conf, instance_id, dplx, dplx/pixel_per_nm) if save_conf else (cls, *xywh, instance_id, dplx, dplx/pixel_per_nm)  # label format
                        # print(dplx/pixel_per_nm)
                        # print(line)
...
def parse_opt():
    ...
    parser.add_argument('--pixel-per-nm', type=float, default=0, help='the scale between pixel and nm')
